 ...
 */
import { SERVICES, SITE, TELEGRAM, LANGUAGES, localize } from "../public/config.js";
...
export const MENU_KEYBOARD = {
  keyboard: [[{ text: MENU.today }, { text: MENU.tomorrow }]],
...
export async function buildDayList(date, store, title) {
  ...
  const bookings = Object.entries(await store.list(date))
    .map(([time, raw]) => {
      try {
